    ...
    async  getData(){
      ...
      async handleSubmit() {
        ...
        if (this.data.name === "") {
          ...
          this.SET_LOADING_STATUS(true);
          let res = await axios.post("http://127.0.0.1:8000/api/create-category",formData,config);
          this.SET_LOADING_STATUS(false);
  
          if(res.status === 200){
              useToast().success("Category Created");
              document.getElementById("closeBtn").click()
              this.data.name = ""
              this.data.image = ""
              this.getData();
          }else{
              useToast().error("Catgory Can't Create");
          }
        }
      },
      onFileChange(event) {
      ...
      async deleteItem(id,image){
          this.SET_LOADING_STATUS(true);
          let res = await axios.post("http://127.0.0.1:8000/api/delete-category",{id: id,image: image});
          this.SET_LOADING_STATUS(false);
